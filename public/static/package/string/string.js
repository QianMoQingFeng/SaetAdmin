      ...
      */
      ...
      !(function () {
        ...
        var __sp = S.prototype = {
          ...
          random: function (len, parse = false) {
            let str = ''
            // 大写字母
            if (parse) {
              let arr = new S(this.s).parseCSV('+', '');
              if (arr.indexOf('LETTER') > -1 || arr.indexOf('Letter') > -1) for (var i = 0; i < 26; i++)str += String.fromCharCode(65 + i);
              if (arr.indexOf('letter') > -1 || arr.indexOf('Letter') > -1) for (var i = 0; i < 26; i++)str += String.fromCharCode(97 + i);
              if (arr.indexOf('number') > -1) for (var i = 0; i < 10; i++) str += i;
            } else str = this.s;
            let res = '';
            for (let i = 0; i < len; i++) res += str.charAt(Math.floor(Math.random() * str.length))
            return res
          }
        }
