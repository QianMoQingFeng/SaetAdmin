<?php /*a:3:{s:64:"/media/psf/qianmokeji/SaetAdmin/saet.io/saet/install/install.vue";i:1668783918;s:27:"../saet/install/layout.html";i:1668778145;s:36:"../app/admin/view/public/assgin.html";i:1667877771;}*/ ?>
<!DOCTYPE html>
<html lang="cn">
...
<head>
  ...
  <script>
    const theheColor = 'purple';
    let THEME = window.matchMedia("(prefers-color-scheme: dark)").matches ? 'dark' : 'light';
    const htmlDom = document.getElementsByTagName('html')[0]
    htmlDom.setAttribute('class', `${THEME} ${theheColor}`)
...
<body>
  ...
  <vue><template>
    <div class="min-body">
        ...
        <div class="st-flex justify-center st-m-b-30"><?php echo lang('How can we go far without sailing'); ?> ✨</div>
        ...
        <div class="st-flex">
                ...
                <template #default>
                    <el-space :size="20" direction="vertical" fill style="height: 100%;">
                        <div class="st-m-t-20" style="font-weight:500;color: var(--el-text-color-primary);">
                            SaetAdmin <?php echo lang('Development framework'); ?></div>
                        <div> SaetAdmin <?php echo lang('is a service development framework inclined to automation (SAET)'); ?> ^-^ </div>
                        <div>SaetAdmin <?php echo lang('is still adhering to the principle of letting users'); ?><span style="color:red;">S</span>imple <span style="color:red;">a</span>nd
                            <span style="color:red;">e</span>legan<span style="color:red;">t</span> <?php echo lang('to develop'); ?>
                        </div>
                        <div> <?php echo lang('Version'); ?>：{{ ST.version }}</div>
                        <div class="line"> <span> <?php echo lang('Doc'); ?>：</span>
                        ...
                        <div class="line"> <span> <?php echo lang('WebSite'); ?>：</span>
                            <el-link class="light-color" style="font-size:12px;font-weight: normal;" target="_blank"
            ...
            <el-space direction="vertical" style="width:100%;" fill>
                <el-steps :active="stepCur" simple finish-status="success">
                    <el-step title="<?php echo lang('check'); ?>" :icon="stepCur > 0 ? 'select' : stepCur == 0 ? 'edit' : 'none'"></el-step>
                    <el-step title="<?php echo lang('install'); ?>" :icon="stepCur > 1 ? 'select' : stepCur == 1 ? 'edit' : 'none'"></el-step>
                    <el-step title="<?php echo lang('complete'); ?>" icon="none"></el-step>
                </el-steps>


                <template v-if="stepCur == 0">
                    <el-table :data="checkData" border style="width: 100%;border-radius:8px;overflow: hidden;">
                        <el-table-column prop="app" label="<?php echo lang('app'); ?>"></el-table-column>
                        <el-table-column prop="version_keep" label="<?php echo lang('version need'); ?>"></el-table-column>
                        <el-table-column prop="version" label="<?php echo lang('now version'); ?>"></el-table-column>
                        <el-table-column prop="status" label="<?php echo lang('status'); ?>" fixed="right" width="90px">
                            <template #default="{ row }">
                                <template v-if="row.status == 'yes'">
                                    <saet-icon name="ri-checkbox-circle-fill" style="color:var(--el-color-success)"
                                        :size="16"> </saet-icon>
                                    <span class="text"> <?php echo lang('pass'); ?></span>
                                </template>
                                <template v-if="row.status == 'fail'">
                                    <saet-icon name="ri-close-circle-fill" style="color:var(--el-color-error)"
                                        :size="16">
                                    </saet-icon>
                                    <span class="text"> <?php echo lang('mismatch'); ?></span>
                                </template>
                                <template v-if="row.status == 'wait'">
                                    <saet-icon name="ri-information-fill" style="color:var(--el-color-warning)"
                                        :size="16">
                                    </saet-icon>
                                    <span class="text"> <?php echo lang('wait check'); ?></span>
                                </template>
                            </template>
                        </el-table-column>
                    ...
                    <el-form ref="formRef" :model="form" label-width="90px" :label-position="position" :rules="rules">
                        <div class="title"><?php echo lang('database information'); ?></div>
...
SaetApp({
    setup() {
        ...
        },
        {
            app: 'mysql',
            version_keep: '>= 5.7',
            version: '<?php echo lang("wait check"); ?>',
            status: 'wait'
        },
        {
            app: '<?php echo lang("file right"); ?>',
            version_keep: '>= 0755',
  ...
  <script type="module">
    ...
    window.matchMedia('(prefers-color-scheme: dark)')
      .addEventListener('change', event => {
        if (event.matches) {
          htmlDom.setAttribute('class', `dark ${theheColor}`)
        } else {
          htmlDom.setAttribute('class', `light ${theheColor}`)
        }
      })
